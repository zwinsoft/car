...
function isDisabled($m)
{
    ...
    if($data)
    {
        if($data['status'] <=3)
        {
            return "class='disabled' title='请先提交企业承诺书，方可进行后续步骤填写！'";
        }
    }
    return $m;
}

function isReviewerDisabled($m)
{
    $data = M('Reviewer')->where('uid='.session('user_id'))->find();
    if($data)
    {
        if($data['commitFlag'] !=1)
        {
            return "class='disabled' title='请先提交评审承诺书，方可进行后续步骤填写！'";
        }
    }
    return $m;
}
...
function download_pdf($pdfLocalUrl,$mime,$url)
{
    $return_content = request_by_curl($url);
    $fp= @fopen($pdfLocalUrl,"w"); //将文件绑定到流  
